...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) {
    ...
    $sql = "SELECT * FROM scan_users WHERE username=? AND company_code=?";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($password === $user['password']) {
            $_SESSION['scan_user'] = $user['username'];
...
<head>
    ...
    <title>Login - Scan</title>
</head>
<body>
    <div class="login-container">
        <h1>Login to Access Scan</h1>
        <?php if ($login_error): ?>
        ...
        <p>Don't have an account? <a href="scan_signup.php">Sign up here</a></p>
    </div>
</body>
</html>
